...
function getImage(){const bgImage=document.getElementById('bg');let locations;const lang=getLang();$.getJSON('js/locations.json',function(data,status,xhr){if(xhr.status>=200&&xhr.status<400){locations=data.locations
const objLength=locations.length;const item=Math.floor((Math.random()*objLength));const imageId=locations[item].id;bgImage.style.backgroundImage=locations[item].image;let locationName=(lang==='en')?locations[item].place.en:locations[item].place.es;let location=locations[item].location;let fact=(lang==='en')?locations[item].fact.en:locations[item].fact.es;let link=(lang==='en')?locations[item].link.en:locations[item].link.es;let owner=locations[item].owner;let url=locations[item].url;let photo=(lang==='en')?'Photograph':'Fotografia';let locationIcon='<i class="fas fa-map-marker-alt"></i>';document.getElementById('locationName').innerHTML=`${locationName}`;document.getElementById('locationUrl').setAttribute('href',link);document.getElementById('location').innerHTML=`${locationIcon} ${location}`;document.getElementById('fact').innerHTML=fact;document.getElementById('owner').innerHTML=owner;document.getElementById('taker').setAttribute('href',url);document.getElementById('photo').innerHTML=photo;const reloadImage=setTimeout(getImage,60000)}else{console.log(`error : status => ${status}`)}})};function startTime(){const clock=document.getElementById('clock');const greetElement=document.getElementById('greet');const today=new Date();const h=today.getHours();let m=today.getMinutes();let s=today.getSeconds();m=checkTime(m);s=checkTime(s);clock.innerHTML=`${h}:${m}`;const reloadTime=setTimeout(startTime,500)};function checkTime(i){if(i<10){i=`0${i}`};return i};function getLang(){let lang='en';let browserLang=navigator.language;if(browserLang.includes('es')){return lang='es'}
return lang}})()