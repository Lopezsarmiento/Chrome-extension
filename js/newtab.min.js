...
function getWeather(lat,long){const apiKey='&appid=cf6f3902316f9fa78adcc4f336e2728a';const latlong=`lat=${lat}&lon=${long}`;const units='&units=metric';const url=`http://api.openweathermap.org/data/2.5/weather?${latlong}${units}${apiKey}`;const cityId=document.getElementById('city');const tempId=document.getElementById('temp');const iconId=document.getElementById('weatherIcon');fetch(url).then(function(response){if(!response.ok){throw Error(response.statusText)}
...
const temperature=(data.main.temp).toFixed(0);tempId.innerHTML=` | ${temperature}&#x2103;`;iconId.setAttribute('class',weatherIconID);cityId.innerHTML=data.name}).catch(function(error){console.log('Looks like there was a problem: \n',error)})}
